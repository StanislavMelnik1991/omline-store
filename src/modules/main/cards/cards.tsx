...
export function Cards() {
	...
	return (
		<div className={style.wrapper}>
			<SortProducts />
			<div className={style.incorrect_filters} 
				style={{ display: `${productsForRender.length === 0 ? 'flex' : 'none'}` }}>
			...
			<div className={style.pagination}>
				...
				<button style={st}
					...
					disabled={isLoading}>
					<InlineSVG src={require('../../../assets/to_left.svg')}
						className={style.logo} />
				</button>
				<p>{page}</p>
				<button style={st}
					...
					onClick={() => dispatch(pagination(1))}
					disabled={isLoading}>
					<InlineSVG src={require('../../../assets/to_right.svg')}
						className={style.logo} />
				</button>
				<button
					style={st}
